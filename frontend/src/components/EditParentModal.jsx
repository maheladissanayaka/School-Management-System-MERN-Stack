...
const EditParentModal = ({ isOpen, onClose, refreshData, parent }) => {
  ...
  useEffect(() => {
    if (parent && isOpen) {
      setFormData({
        ...
        image: parent.image || '',
        students: parent.students ? parent.students.map(s => s._id) : [] 
      });
      setSelectedStudentsData(parent.students || []); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateParent(parent._id, cleanData);
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
      ...
      <div className="bg-white rounded-[2rem] w-full max-w-4xl shadow-2xl p-8 relative max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <h2 className="text-2xl font-black text-gray-800 mb-6 flex items-center gap-2">
            <User className="text-blue-600" /> Edit Parent Record
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
           ...
           <div className="flex flex-col items-center justify-center mb-4">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase mt-2">Update Photo</span>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                  ...
                  <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="input-field" placeholder="Parent Name" />
              </div>
              ...
              <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-1 ml-1">Relationship Type</label>
              ...
              <div className="flex flex-wrap gap-2 mb-4 min-h-[30px]">
                {selectedStudentsData.length > 0 ? selectedStudentsData.map(student => (
                   <div key={student._id} className="flex items-center gap-2 bg-white pl-2 pr-1 py-1 rounded-full border border-blue-200 shadow-sm animate-in fade-in zoom-in duration-200">
                      ...
                      <span className="font-bold text-xs text-gray-700">{student.name}</span>
                    ...
                    <span className="text-xs text-gray-400 italic">No students linked yet. Use search below.</span>
                )}
              </div>

              <div className="relative">
                 <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                 <input type="text" placeholder="Search to add more students..." className="w-full pl-10 pr-4 py-3 rounded-xl border-none outline-none focus:ring-2 focus:ring-blue-500 font-medium placeholder:text-gray-400 bg-white"
                    value={studentSearch} onChange={e => setStudentSearch(e.target.value)}
                 />
                 {studentSearch && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl z-20 border border-gray-100 max-h-48 overflow-y-auto no-scrollbar">
                        {filteredStudents.length > 0 ? filteredStudents.map(student => (
                            <div key={student._id} onClick={() => handleAddStudent(student)} className="flex items-center gap-3 p-3 hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-none">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-400 font-medium">{student.studentId}</p>
                                </div>
                            </div>
                        )) : (
